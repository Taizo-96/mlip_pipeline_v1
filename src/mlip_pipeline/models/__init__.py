"""Top-level models package for mlip_pipeline.

All shared data-model dataclasses live here so every sub-package
(validate, train, select, …) can import from a single location:

    from mlip_pipeline.models import ValidationResult, EosResult, …
"""
...
class MeltingResult:
    ...
    structure_id: str
    T_melt: float = float("nan")        # K
    T_bracket_lo: float = float("nan") # K  lowest T where liquid grew
    T_bracket_hi: float = float("nan") # K  highest T where solid grew
...
class ThermalExpansionResult:
    ...
    structure_id: str
    temperatures: list[float] = field(default_factory=list)  # K
    volumes: list[float] = field(default_factory=list)       # Å³/atom
    alpha: float = float("nan")   # K⁻¹  linear thermal expansion coefficient
    V_ref: float = float("nan")   # Å³/atom at T_ref
    T_ref: float = float("nan")   # K
    compute_ok: bool = False
    error: Optional[str] = None
...
class RdfResult:
    ...
    structure_id: str
    r: list[float] = field(default_factory=list)    # Å
    g_r: list[float] = field(default_factory=list)  # g(r)
    temperature: float = float("nan")               # K
    first_peak_r: float = float("nan")              # Å  position of first peak
    first_peak_g: float = float("nan")              # height of first peak
    compute_ok: bool = False
    error: Optional[str] = None
