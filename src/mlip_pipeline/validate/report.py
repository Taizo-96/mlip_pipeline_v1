...
Both files are written to ``out_dir`` as:
...
Row = dict[str, str]  # ordered dict of column -> value


def _collect_eos(
    mtp: ValidationResult,
    ref: "ClassicalReferenceResult",
) -> list[Row]:
    ...
    for mr in mtp.eos_results:
        ...
        rows.append({
            "property": "EOS",
            "structure": mr.structure_id,
            "quantity": "V0 (Å³/atom)",
            "MTP": _fmt(mr.V0, 4) if mr.fit_ok else "—",
            "reference": _fmt(rr.V0, 4) if (rr and rr.fit_ok) else "—",
            "delta_pct": _pct(mr.V0, rr.V0) if (mr.fit_ok and rr and rr.fit_ok) else "n/a",
        })
        rows.append({
            "property": "EOS",
            "structure": mr.structure_id,
            "quantity": "B0 (GPa)",
            "MTP": _fmt(mr.B0, 2) if mr.fit_ok else "—",
            "reference": _fmt(rr.B0, 2) if (rr and rr.fit_ok) else "—",
            "delta_pct": _pct(mr.B0, rr.B0) if (mr.fit_ok and rr and rr.fit_ok) else "n/a",
        })
        rows.append({
            "property": "EOS",
            "structure": mr.structure_id,
            "quantity": "B0' (—)",
            "MTP": _fmt(mr.B0p, 2) if mr.fit_ok else "—",
            "reference": _fmt(rr.B0p, 2) if (rr and rr.fit_ok) else "—",
            "delta_pct": _pct(mr.B0p, rr.B0p) if (mr.fit_ok and rr and rr.fit_ok) else "n/a",
        })
    return rows
...
def _collect_elastic(
    ...
    for mr in mtp.elastic_results:
        ...
        # Cij constants
        all_keys = sorted(
            set((mr.C or {}).keys()) | set(((rr.C if rr else None) or {}).keys())
        )
        for k in all_keys:
            mv = (mr.C or {}).get(k)
            ...
            rows.append({
                ...
                "structure": mr.structure_id,
                "quantity": f"{k} (GPa)",
                "MTP": _fmt(mv, 2) if mv is not None else "—",
                "reference": _fmt(rv, 2) if rv is not None else "—",
                "delta_pct": _pct(mv, rv) if (mv is not None and rv is not None) else "n/a",
            })
        # Aggregate moduli
...
def _collect_melting(
    ...
    for mr in mtp.melting_results:
        ...
        rows.append({
            ...
            "structure": mr.structure_id,
            "quantity": "T_melt (K)",
            "MTP": _fmt(mr.T_melt, 0) if mr.compute_ok else "—",
            "reference": _fmt(rr.T_melt, 0) if (rr and rr.compute_ok) else "—",
            "delta_pct": _pct(mr.T_melt, rr.T_melt) if (mr.compute_ok and rr and rr.compute_ok) else "n/a",
        })
    return rows
...
def _collect_vacancy(
    ...
    for mr in mtp.vacancy_results:
        ...
        rows.append({
            ...
            "structure": mr.structure_id,
            "quantity": "E_vac (eV)",
            "MTP": _fmt(mv, 4) if mv is not None else "—",
            "reference": _fmt(rv, 4) if rv is not None else "—",
            "delta_pct": _pct(mv, rv) if (mv is not None and rv is not None) else "n/a",
        })
    return rows
...
def _collect_rdf(
    ...
    for mr in mtp.rdf_results:
        ...
        rows.append({
            ...
            "structure": mr.structure_id,
            "quantity": "r_1 (Å)",
            "MTP": _fmt(mv, 3) if mv is not None else "—",
            "reference": _fmt(rv, 3) if rv is not None else "—",
            "delta_pct": _pct(mv, rv) if (mv is not None and rv is not None) else "n/a",
        })
    return rows
...
def _md_table(rows: list[Row]) -> list[str]:
    """Render a list of Row dicts as a GFM Markdown table."""
...
def write_report(
    ...
    """Write validation_report.md and validation_summary.csv to out_dir.
    ...
    section_rows: list[tuple[str, list[Row]]] = [
        (title, fn(mtp_result, ref_result))
